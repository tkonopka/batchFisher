...
test_that("batch calculation should use relevant precomputed values", {
  ...
  # all p-values that have count_00==100 should have realistic de-novo computed p-values
  good.p = result[count_00==100]$p.value
  expect_true(all(good.p > 0 & good.p < 1))
  # all p-values that have count_00==50 should come from precomp, which was corrupted
